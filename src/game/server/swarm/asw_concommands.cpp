...
void rm_hordescalef(const CCommand &args)
{
	...
	if (!ASWGameRules() || (ASWGameRules()->GetGameState() != ASW_GS_BRIEFING))
	{
		Msg("Horde scale can only be enabled during briefing \n");
		return;
	}
	...
	if (pPlayer && ASWGameResource() && ASWGameRules())
	{
		if (ASWGameResource()->m_Leader.Get() != pPlayer)
		{
			Msg("Only leader can enable horde scale\n");
			return;
		}

		int multiplier = clamp(atoi(args[1]), 1, 15);
		ASWGameRules()->m_iHordeScale = multiplier;
		//ASW_ApplyCarnage_f(multiplier);

		CReliableBroadcastRecipientFilter filter;
		char buffer[512];
		Q_snprintf(buffer, sizeof(buffer), "Horde scale activated with multiplier: %i", multiplier);
		UTIL_ClientPrintFilter(filter, ASW_HUD_PRINTTALKANDCONSOLE, buffer);
	}
}
ConCommand rm_hordescale("rm_hordescale", rm_hordescalef, "Scales the horde size", 0);
...
void rm_ammo_bonusf(const CCommand &args)
{
	...
	if (pPlayer && ASWGameResource() && ASWGameRules())
	{
		...
		int ammo_count = clamp(atoi(args[1]), 0, 60);
